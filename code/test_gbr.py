...
# 2. Settings & Argparse
# =============================
parser = argparse.ArgumentParser(description="Evaluate GBT VRP travel-time estimator.")
...
parser.add_argument("--modeltag", type=str, default="gbr_all",
                    help="Suffix tag used when training (default: 'gbr_all').")
...
# Derive input mask just like in training
...
def evaluate_array(model, X_np, y_np, name_suffix):
    ...
    diffs = y_pred - y_true
    reldiffs = np.abs(diffs) / np.maximum(np.abs(y_true), 1e-6)  # avoid div by zero

    mae  = mean_absolute_error(y_true, y_pred)
    rmse = sqrt(mean_squared_error(y_true, y_pred))
    mean_reldiff = np.mean(reldiffs) if len(reldiffs) else np.nan
    percentiles_reldiff = np.percentile(reldiffs, [10, 50, 90]) if len(reldiffs) else np.array([np.nan]*3)
...
if __name__ == "__main__":
    ...
    # Build unseen subsets and groups
    ...
    # Load saved GBT model (note the "gbr_" prefix in the filename)
    ...
    for data_array, name_suffix in test_sets:
        ...
        # Explicit MAE/RMSE print (already printed inside evaluate_array)
